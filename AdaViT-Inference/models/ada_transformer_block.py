...
class SimpleTokenSelect(nn.Module):
    ...
    def forward(self, x, attn, attn_pre_softmax, token_select=None):
        ...
        if self.pre_softmax or not self.training :
            eye_mat = torch.eye(l, device=attn.device, dtype=attn.dtype)
            
            mask_term = (1 - attn_policy - eye_mat)
            # Force it to be clean tensor
            mask_term = mask_term * 1.0
            
            # We want to add -1e9 where mask_term > 0.
            # Assuming mask_term is either 0 or 1 (approx).
            # Let's relu it to be safe.
            mask_term = torch.relu(mask_term)
            
            # Add large negative bias
            attn = attn_pre_softmax * attn_policy + mask_term * -1e9
