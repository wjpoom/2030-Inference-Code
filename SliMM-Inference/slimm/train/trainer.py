...
class SliMMTrainer(Trainer):
    ...
    def create_optimizer(self):
        ...
        if self.optimizer is None:
            ...
            if self.args.mm_vision_tower_lr is not None:
                # lr_mapper["visual"] = self.args.mm_vision_tower_lr
                lr_mapper["visual"] = self.args.mm_vision_tower_lr
            if self.args.mm_projector_lr is not None:
                lr_mapper["visual.merger"] = self.args.mm_projector_lr
                # lr_mapper["deepstack"] = self.args.mm_projector_lr
            if len(lr_mapper) > 0:
    ...
    def _save_checkpoint(self, model, trial, metrics=None):
        # FIXME: directly save whole ckpt for convenient
        super(SliMMTrainer, self)._save_checkpoint(model, trial)

    def _save(self, output_dir: Optional[str] = None, state_dict=None):
        # FIXME: directly save whole ckpt for convenient
        super(SliMMTrainer, self)._save(output_dir, state_dict)
