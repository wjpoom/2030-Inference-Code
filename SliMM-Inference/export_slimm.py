...
path = r'/path/to/SliMM-Qwen2-0.5B'             # Set the folder path where the Qwen2.5-VL whole project downloaded.
...
onnx_model_F = r'/mnt/public/users/chenyitong/projects/SliMM/slimm_onnx_2/SliMM_F.onnx'  # You must assign a different folder path to avoid duplicate weight file names, which would cause loading failures.

image_path = r"demo.jpeg"                                      # Test image for the exported onnx model.
...
class QwenVL_PartB(torch.nn.Module):
    def __init__(self, qwenvl):
        ...
        self.head_dim = self.qwenvl.config.vision_config.embed_dim // self.num_heads
...
with torch.inference_mode():
    model = SliMMForConditionalGeneration.from_pretrained(path, torch_dtype=torch.float32, device_map="cpu", low_cpu_mem_usage=True).eval()
